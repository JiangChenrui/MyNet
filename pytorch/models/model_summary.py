...
def print_model_parm_flops(model):
    ...
    total_flops = (
        sum(list_conv) + sum(list_linear) + sum(list_bn) + sum(list_relu) +
        sum(list_pooling))
            ...
            def hook(module, input, output):
                ...
                if input_shape:
                    summary[m_key]['input_shape'] = (-1,) + tuple(
                        input[0].size())[1:]
                summary[m_key]['output_shape'] = (-1,) + tuple(
                    output.size())[1:]
                if weights:
...
def show_save_tensor(model):
    ...
    def save_tensor(tensor,
        ...
        if all_kernels:
            ...
            tensor = tensor[:, ch, :, :].unsqueeze(dim=1)
        utils.save_image(
            tensor, filename, nrow=nrow, normalize=True, padding=padding)
